...
def do_python_eval(output_dir='output', use_07=True):
    ...
    for i, cls in enumerate(labelmap):
        ...
        plt.plot(rec*100,prec*100,label=cls)
    ...
    R=sum(rec_l)/len(rec_l)
    P=sum(pre_l) / len(pre_l)
    print('Mean Recall={:.4f}'.format(R))
    print('Mean Precision={:.4f}'.format(P))
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    time2=time.clock()
    print("detecting %d images needs %.4fs"%(num_images,(time2-time1)))
    ...
    # flops,param=profile(net,torch.rand((1, 3, 320, 320)))
    print('--------------------------------------------------------------')

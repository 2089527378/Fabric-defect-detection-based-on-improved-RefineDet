...
class RefineDet_se(nn.Module):
    """Single Shot Multibox Architecture adding SE block
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        super(RefineDet_se, self).__init__()
        self.phase = phase
        ...
        self.list_se=nn.ModuleList([SE(512,16),SE(512,16),SE(1024,16),SE(512,16),SE(256,16),SE(256,16),SE(256,16),SE(256,16),])



        if phase == 'test':
            self.softmax = nn.Softmax(dim=-1)
            # self.detect = Detect_RefineDet_softnms(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.01, 500)
            self.detect = Detect_RefineDet(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.08, 500)
...
def build_refinedet_se(phase, size=320, num_classes=21):
    ...
    return RefineDet_se(phase, size, base_, extras_, ARM_, ODM_, TCB_, num_classes)

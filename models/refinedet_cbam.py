...
class RefineDet_cabm(nn.Module):
    """Single Shot Multibox Architecture adding CBAM block
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        super(RefineDet_cabm, self).__init__()
        self.phase = phase
        ...
        self.list_cbam = nn.ModuleList(
            [CBAM(512, 16), CBAM(512, 16), CBAM(1024, 16), CBAM(512, 16), CBAM(256, 16), CBAM(256, 16), CBAM(256, 16),
             CBAM(256, 16), ])
    ...
    def forward(self, x):
        ...
        for k in range(30):
            ...
            if 22 == k:
                ...
                s =self.list_cbam[0](s)
                sources.append(s)
            elif 29 == k:
                s = self.conv5_3_L2Norm(x)
                s = self.list_cbam[1](s)
                sources.append(s)

        # apply vgg up to fc7
        for k in range(30, len(self.vgg)):
            x = self.vgg[k](x)
        s = self.list_cbam[2](x)
        sources.append(s)
...
def build_refinedet_cbam(phase, size=320, num_classes=21):
    ...
    return RefineDet_cabm(phase, size, base_, extras_, ARM_, ODM_, TCB_, num_classes)

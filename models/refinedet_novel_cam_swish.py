...
import os
from .CAM import *
# from .beta_mish import beta_mish
from torchtoolbox.nn import Activation


class RefineDet_novel_cam_swish(nn.Module):
    """Single Shot Multibox Architecture adding BA-TCB, CAM block and Swish Activation function
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB,TCB_add, num_classes):
        super(RefineDet_novel_cam_swish, self).__init__()
        self.phase = phase
        ...
        self.tcb_0 = nn.ModuleList(TCB_add[0])
        self.tcb_1 = nn.ModuleList(TCB_add[1])
        self.tcb_2 = nn.ModuleList(TCB_add[2])

        self.list_cam = nn.ModuleList(
            [CAM(512, 16), CAM(512, 16), CAM(1024, 16), CAM(512, 16), CAM(256, 16), CAM(256, 16), CAM(256, 16),
             CAM(256, 16), ])

        if phase == 'test':
            self.softmax = nn.Softmax(dim=-1)
            self.detect = Detect_RefineDet(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.03, 500)
    ...
    def forward(self, x):
        ...
        for k, v in enumerate(sources[::-1]):
            ...
            p = s
            s = self.list_cam[k + 4](s)
            tcb_source.append(s)
        #print([x.size() for x in tcb_source])
        tcb_source.reverse()

        #apply PANet
        for k, v in enumerate(tcb_source):
            s=v
            for i in range(3):
                s = self.tcb_0[k*3 + i](s)
                #print(s.size())
            if k != 0:
                u = p
                u = self.tcb_1[k-1](u)
                s += u
            for i in range(3):
                s = self.tcb_2[k*3 + i](s)
            p = s
            tcb_add_source.append(s)

        # apply ODM to source layers
        for (x, l, c) in zip(tcb_add_source, self.odm_loc, self.odm_conf):
...
def add_tcb(cfg):
    ...
    for k, v in enumerate(cfg):
        feature_scale_layers += [nn.Conv2d(cfg[k], 256, 3, padding=1),
                                 Activation('h_swish', auto_optimize=True),
                                 nn.Conv2d(256, 256, 3, padding=1)
        ]
        feature_pred_layers += [ Activation('h_swish', auto_optimize=True),
                                nn.Conv2d(256, 256, 3, padding=1),
                                 Activation('h_swish', auto_optimize=True)
        ]
        if k != len(cfg) - 1:
...
def add_tcb_(cfg):
    ...
    for k, v in enumerate(cfg):
        ...
        if k != 0:
            feature_upsample_layers += [nn.Conv2d(256, 256, 3, 2, 1)]
    return (feature_scale_layers, feature_upsample_layers, feature_pred_layers)
...
def build_refinedet_novel_cam_swish(phase, size=320, num_classes=21):
    ...
    TCB_add = add_tcb_(tcb_add[str(size)])
    return RefineDet_novel_cam_swish(phase, size, base_, extras_, ARM_, ODM_, TCB_,TCB_add, num_classes)

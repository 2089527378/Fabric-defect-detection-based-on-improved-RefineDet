...
from .beta_mish import *
from layers import *
from data import voc_refinedet, coco_refinedet
import os


class RefineDet_mish(nn.Module):
    """Single Shot Multibox Architecture adding Mish Activation function
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        super(RefineDet_mish, self).__init__()
        self.phase = phase
...
def vgg(cfg, i, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, nn.BatchNorm2d(v), beta_mish()]
            else:
                layers += [conv2d, beta_mish()]
            in_channels = v
    pool5 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
    conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=3, dilation=3)
    conv7 = nn.Conv2d(1024, 1024, kernel_size=1)
    layers += [pool5, conv6,
               beta_mish(), conv7, beta_mish()]
    return layers
...
def build_refinedet_mish(phase, size=320, num_classes=21):
    ...
    return RefineDet_mish(phase, size, base_, extras_, ARM_, ODM_, TCB_, num_classes)

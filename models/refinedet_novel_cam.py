...
class RefineDet_novel_cam(nn.Module):
    """Single Shot Multibox Architecture adding BA-TCB and CAM block
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB,TCB_add, num_classes):
        super(RefineDet_novel_cam, self).__init__()
        self.phase = phase
        ...
        if phase == 'test':
            ...
            self.detect = Detect_RefineDet_diounms(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.09, 500)
...
def build_refinedet_novel_cam(phase, size=320, num_classes=21):
    ...
    return RefineDet_novel_cam(phase, size, base_, extras_, ARM_, ODM_, TCB_,TCB_add, num_classes)

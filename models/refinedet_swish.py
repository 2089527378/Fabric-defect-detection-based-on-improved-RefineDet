...
class RefineDet(nn.Module):
    """Single Shot Multibox Architecture adding Swish Activation function
...
def vgg(cfg, i, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, nn.BatchNorm2d(v), Activation('relu', auto_optimize=True)]
            else:
                layers += [conv2d,  Activation('relu', auto_optimize=True)]
            in_channels = v
    pool5 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
    conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=3, dilation=3)
    conv7 = nn.Conv2d(1024, 1024, kernel_size=1)
    layers += [pool5, conv6,
               Activation('relu', auto_optimize=True), conv7,  Activation('relu', auto_optimize=True)]
    return layers
...
def build_refinedet_swish(phase, size=320, num_classes=21):

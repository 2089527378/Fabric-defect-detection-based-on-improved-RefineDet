...
class RefineDet_novel_cam_mish(nn.Module):
    """Single Shot Multibox Architecture adding BA-TCB, CAM block and Mish Activation function
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB,TCB_add, num_classes):
        super(RefineDet_novel_cam_mish, self).__init__()
        self.phase = phase
...
def add_tcb(cfg):
    ...
    for k, v in enumerate(cfg):
        ...
        feature_pred_layers += [beta_mish(1.5),
                                nn.Conv2d(256, 256, 3, padding=1),
                                beta_mish(1.5)
        ]
        if k != len(cfg) - 1:
...
def build_refinedet_novel_cam_mish(phase, size=320, num_classes=21):
    ...
    return RefineDet_novel_cam_mish(phase, size, base_, extras_, ARM_, ODM_, TCB_,TCB_add, num_classes)

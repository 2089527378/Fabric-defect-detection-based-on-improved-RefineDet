...
class RefineDet_novel(nn.Module):
    """Single Shot Multibox Architecture adding BA-TCB structure
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB,TCB_add, num_classes):
        super(RefineDet_novel, self).__init__()
        self.phase = phase
        self.num_classes = num_classes
        self.cfg = (coco_refinedet, voc_refinedet)[num_classes == 11]
        ...
        if phase == 'test':
            ...
            self.detect = Detect_RefineDet(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.03, 200)
...
def build_refinedet_novel(phase, size=320, num_classes=21):
    ...
    return RefineDet_novel(phase, size, base_, extras_, ARM_, ODM_, TCB_,TCB_add, num_classes)

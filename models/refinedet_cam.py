...
class RefineDet_cam(nn.Module):
    """Single Shot Multibox Architecture adding CAM block
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        super(RefineDet_cam, self).__init__()
        self.phase = phase
        ...
        self.list_cam = nn.ModuleList([CAM(512,16),CAM(512,16),CAM(1024,16),CAM(512,16),CAM(256,16),CAM(256,16),CAM(256,16),CAM(256,16),])


        if phase == 'test':
            self.softmax = nn.Softmax(dim=-1)
            # self.detect = Detect_RefineDet(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.03, 500)
            self.detect = Detect_RefineDet_diounms(num_classes, self.size, 0, 1000, 0.01, 0.45, 0.01, 500)
    def forward(self, x):
...
def build_refinedet_cam(phase, size=320, num_classes=21):
    ...
    return RefineDet_cam(phase, size, base_, extras_, ARM_, ODM_, TCB_, num_classes)

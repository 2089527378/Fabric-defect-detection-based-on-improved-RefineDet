...
def transform_box(box,transforms):
    ...
    for operate in transforms:
        ...
        if 'resize' == operate['opt']:
            transformed_box = resize_box(transformed_box,operate['fx'],operate['fy'])
        elif 'rotate' == operate['opt']:
            #box,cterxy,imgwh,rot_angle,scale=1.0,correction=True
            tmp_box = rot_box(transformed_box,operate['cterxy'],operate['imgwh'],operate['rot_angle'],operate['scale'],operate['correction'])
            ...
            transformed_box=crop_box(transformed_box,operate['top_left_x'],operate['top_left_y'],operate['crop_w'],operate['crop_h'],operate['iou_thr'])
        elif 'flip' == operate['opt']:
            transformed_box = flip_box(transformed_box,operate['imgwh'][0],operate['imgwh'][1],operate['flip_type'])
...
def transform_img(src_img,transforms):
    ...
    for operate in certain_transforms:
        ...
        if 'rotate' == operate['opt']:
            ...
            transformed_img=rot_img_and_padding(transformed_img,bk_img,cterxy,rot_angle,operate['scale'])
            ...
            transformed_img = resize_img(transformed_img,operate['dsize'],operate['fx'],operate['fy'])
            ...
            transformed_img = crop_img(transformed_img,top_left_x,top_left_y,crop_imgw,crop_imgh)
            ...
            transformed_img=flip_img(transformed_img,flip_type)
...
def main():
    imgs_dir ='DAGMdatasets\VOCdevkit\VOC2007\JPEGImages/'
    bk_imgs_dir ='DAGMdatasets\VOCdevkit\VOC2007\JPEGImages/'
    xmls_dir = 'DAGMdatasets\VOCdevkit\VOC2007\Annotations/'
    
    imgs_save_dir= 'DAGMdatasets\VOCdevkit\VOC2007\mosaic_imgs/'
    if not os.path.exists(imgs_save_dir):
        os.makedirs(imgs_save_dir)
    xmls_save_dir='DAGMdatasets\VOCdevkit\VOC2007\mosaic_xmls/'
    if not os.path.exists(xmls_save_dir):
        os.makedirs(xmls_save_dir)
    
    name_suffix='mosaic' #命名标识
    count = 1046 #拼接图片总数
    imgw,imgh = 512,512 #每张拼接图的大小
